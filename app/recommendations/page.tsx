...
export default function RecommendationsPage() {
  ...
  const [roadmapSuggestions, setRoadmapSuggestions] = useState<Recommendation[]>(
    []
  );
  const [domainSuggestions, setDomainSuggestions] = useState<Recommendation[]>(
    []
  );
  const [resourceSuggestions, setResourceSuggestions] = useState<Recommendation[]>(
    []
  );
  const [progressGapSuggestions, setProgressGapSuggestions] =
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Fetch all recommendation types concurrently
        const [
          roadmapResult,
          domainResult,
          resourceResult,
          ...
          getRoadmapSuggestions(user.id, supabase),
          getDomainSuggestions(user.id, supabase),
          getResourceSuggestions(user.id, supabase),
          getProgressGapSuggestions(user.id, supabase),
        ]);

        setRoadmapSuggestions(roadmapResult);
        setDomainSuggestions(domainResult);
        setResourceSuggestions(resourceResult);
        setProgressGapSuggestions(progressGapResult);
      } catch (err: unknown) {
    ...
    <div className="min-h-screen bg-dark-bg text-dark-fg p-6 sm:p-8">
      <header className="border-b dark.border-gray-600 mb-6">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <h1 className="text-2xl font-bold tracking-wide">Learning Recommendations</h1>
      ...
      <main className="max-w-7xl mx-auto">
        ...
        {roadmapSuggestions.length > 0 && (
          <Card>
            <h3 className="font-medium text-indigo-300 mb-3">Roadmap Skills</h3>
            <p className="text-sm text-gray-400 mb-4">
              Based on your active roadmaps, here are skills to focus on:
            </p>
            <ul className="space-y-3 max-h-80 overflow-y-auto">
              {roadmapSuggestions.map((rec) => (
                <li key={rec.id} className="p-3 bg-gray-800/50 rounded-lg border-indigo-500/20">
                  <div className="flex justify-between items-start">
                    <span className="font-medium text-gray-200">{rec.title}</span>
                    <span className="text-xs text-indigo-400">{rec.reason}</span>
                  </div>
                  {rec.description && (
        ...
        {domainSuggestions.length > 0 && (
          <Card>
            <h3 className="font-medium text-indigo-300 mb-3">Domain-Based Topics</h3>
            <p className="text-sm text-gray-400 mb-4">
              Based on your cybersecurity interests:
            </p>
            <ul className="space-y-3 max-h-80 overflow-y-auto">
              {domainSuggestions.map((rec) => (
                <li key={rec.id} className="p-3 bg-gray-800/50 rounded-lg border-indigo-500/20">
                  <div className="flex justify-between items-start">
                    <span className="font-medium text-gray-200">{rec.title}</span>
                    <span className="text-xs text-indigo-400">{rec.reason}</span>
        ...
        {resourceSuggestions.length > 0 && (
          <Card>
            <h3 className="font-medium text-indigo-300 mb-3">Resource Suggestions</h3>
            <p className="text-sm text-gray-400 mb-4">
              Learning materials matching your profile:
            </p>
            <ul className="space-y-3 max-h-80 overflow-y-auto">
              {resourceSuggestions.map((rec) => (
                <li key={rec.id} className="p-3 bg-gray-800/50 rounded-lg border-indigo-500/20">
                  <div className="flex justify-between items-start">
                    <span className="font-medium text-gray-200">{rec.title}</span>
                    <span className="text-xs text-indigo-400">{rec.reason}</span>
        ...
        {/* Progress Gap Suggestions */}
        {progressGapSuggestions.length > 0 && (
          <Card>
            <h3 className="font-medium text-indigo-300 mb-3">Progress Gaps</h3>
            <p className="text-sm text-gray-400 mb-4">
              Areas where you can continue growing:
            </p>
            <ul className="space-y-3 max-h-80 overflow-y-auto">
              {progressGapSuggestions.map((rec) => (
                <li key={rec.id} className="p-3 bg-gray-800/50 rounded-lg border-indigo-500/20">
                  <div className="flex justify-between items-start">
                    <span className="font-medium text-gray-200">{rec.title}</span>
                    <span className="text-xs text-indigo-400">{rec.reason}</span>
...
async function getRoadmapSuggestions(
  userId: string,
  supabase: any
...
async function getProgressGapSuggestions(
  userId: string,
  supabase: any
): Promise<Recommendation[]> {
  // Get user's goals
